# Design Agentic Workflows in Daily Work
 ...
 ▪ 产品边界：这次要解决什么问题、不解决什么问题

 ▪ 架构决策：微服务还是模块化单体？是否引入 RAG 知识库？

 ▪ 技术取舍：选哪种数据库、哪种消息队列、是否启用缓存、并发策略等

 ▪ 系统级一致性：跨模块的错误码规范、日志规范、接口风格统一性
 ...
 ⁃ Claude Code 能做的是：给你方案对比、罗列 pros/cons；但拍板的人只能是你

 ⁃ 技术能力不能缺席：你至少要有足够的技术判断力，能看懂代码、理解实现，才能给出精准任务并做出最终决策

 ▫ 2. 第二层：AI 做，你来验收（主工作量层）

 ⁃ 典型内容：

 ▪ 业务代码实现（接口、Service、DAO 等）
 ...
 ⁃ 硬标准：任何一行你最终接受的代码，你都要能说清它在干什么

 ▪ 说不清楚，就说明要么需求没想清楚，要么实现超出了你的控制范围 → 不应该直接接收

 ⁃ 对于应用层项目（如 Hify），这一层占比可以非常大，AI 能承担大量代码工作，但你必须在上层做决策和验收

 ▫ 3. 第三层：AI 全权处理，你只需扫一眼（机械层）

 ⁃ 典型内容：

 ▪ 代码格式化、样板代码生成（boilerplate）

 ▪ 简单重构、重命名、提取方法等

 ▪ 启动脚本、Makefile、基础 CI 脚手架等

 ⁃ 这里你的职责是：快速浏览确认无明显问题即可，不要在这些机械工作上耗费脑力

 ▫ 4. 分工的弹性与项目类型差异

 ⁃ 三层边界不是绝对的，要根据项目类型调整：

 ▪ 应用层项目：第二层比重大，更多业务逻辑可以放心交给 AI 来写

 ▪ 基础设施类项目（分布式系统、存储引擎等）：第一层比重大，核心代码、关键路径最好自己写或深度参与

 ⁃ 有时你“还没完全想清楚要做什么”时，不必等到完全清晰再用 AI：

 ▪ 你可以先让 Claude Code 帮你梳理思路（列出需要的基础组件、可能的架构选型等），再由你做判断和取舍

 ▪ 协作模式可以是“它帮你想，你做裁决”，而不是只能“你想清楚再让它干”

 ---

 第一步，查意图：它做的是不是你让它做的？你打开代码，发现它确实实现了删除接口和关联检查。但它额外做了两件事：加了一个软删除机制（不是真删，标记 deleted=true），还加了一个操作日志记录。软删除合不合理？也许合理。操作日志要不要？可能要。但问题是你没要求。它自作主张扩大了范围。这次看着还行，下次它可能就“顺手”加一些不合适的东西。所以第一步的判断：功能多了，需要砍掉或确认后再保留。第二步，查质量：风格、规范、一致性。你看命名，发现检查关联的方法叫 checkProviderUsage，但项目里其他类似方法都叫 validateXxxBeforeDelete。返回格式上，其他接口返回 Result，它这里直接返回 ResponseEntity。这些不是 bug，功能没问题，但放在整个项目里风格不统一，时间一长，每个模块一种风格，维护起来很痛苦。第三步，查边界：错误处理、异常、潜在风险。你发现一个问题：它检查了“有 Agent 在使用则拒绝删除”，但没有处理并发场景——检查通过后、删除前，刚好有新 Agent 绑定了这个提供商怎么办？另外，传入的 ID 不存在时，它抛了 500 而不是 404。这些边界问题，你可以让 Claude Code 自己再检查一遍——它做细节排查很稳定，不会因为疲劳漏掉。

极客时间版权所有: https://time.geekbang.org/column/article/963290?utm_campaign=geektime_search&utm_content=geektime_search&utm_medium=geektime_search&utm_source=geektime_search&utm_term=geektime_search
