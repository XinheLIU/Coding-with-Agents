...
This repository is organized in three layers: `raw/` (research notes), `wiki/` (documentation), and `book/` (the published course).
...
Levels **1–3** live under `book/en/<level>/` and `book/zh-cn/<level>/` with their chapters. **Levels 4–5** are not in those trees yet.

## Draft chapters (in progress)

Levels **4** and **5** are written first under [`draft/`](book/draft/README.md). That folder holds work-in-progress English and 简体中文 notes until they are promoted into `book/en/` and `book/zh-cn/`. The per-language **table of contents** ([`book/en/SUMMARY.md`](book/en/SUMMARY.md), [`book/zh-cn/SUMMARY.md`](book/zh-cn/SUMMARY.md)) still marks those parts as *coming soon* in the main nav; use [`book/draft/README.md`](book/draft/README.md) for direct links to draft overviews.
